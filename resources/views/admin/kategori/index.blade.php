            ...
            <div class="mb-6 flex justify-end">
                <a href="{{ route('admin.kategori.create') }}" 
                   class="inline-flex items-center py-3 px-6 rounded-full text-white bg-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:outline-none focus:ring-indigo-300 transition duration-300 ease-in-out shadow-lg hover:shadow-xl">
                    <span class="mr-2">{{ __('Buat Kategori') }}</span>
                ...
                @forelse ($kategori as $kategoriItem)
                    <div class="flex items-center justify-between bg-white border border-gray-200 p-4 rounded-lg mb-4 shadow-sm hover:shadow-xl transition-shadow duration-300 ease-in-out">
                        ...
                        <div class="flex items-center gap-4">
                            <img src="{{ Storage::url($kategoriItem->ikon) }}" alt="{{ $kategoriItem->nama }}" 
                                 class="w-16 h-16 rounded-lg object-cover border border-gray-300 shadow-md">
                            <h4 class="text-lg font-semibold text-gray-800">{{ $kategoriItem->nama }}</h4>
                        </div>

                        <!-- Toko Information -->
                        <div class="flex items-center gap-4">
                            <span class="text-gray-500 font-medium">
                                {{ $kategoriItem->toko->nama }} <!-- Display toko name from the relation -->
                            </span>
                        </div>

                        <!-- Action Buttons -->
                        <div class="flex items-center gap-x-3">
                            <a href="{{ route('admin.kategori.edit', $kategoriItem) }}" class="py-2 px-5 rounded-full text-white bg-indigo-700 hover:bg-indigo-800 transition duration-300 ease-in-out shadow-md hover:shadow-lg">
                                Edit
                            </a>
                            <form method="POST" action="{{ route('admin.kategori.destroy', $kategoriItem) }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit" 
